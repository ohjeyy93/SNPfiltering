...
with open(args.filtered, "r") as f1:
   ...
   # print(len(Genelist1))
            ...
            if count==1:
                for x in range(len(Genelist1)):
                    if word1== Genelist1[x] and word== str(int(POSlist1[x])-1) and line.startswith("#")==False:
                        ...
                        POSlist2+=[POSlist1[x]]
                        if line.startswith("#")==False and line.find("AF=")!=-1 and line.find("AN=") !=-1:
                            AFlist1+=[float(line[line.find("AF=")+3:line.find("AN=")-1])]
                        if line.startswith("#")==False and line.find("QD=")!=-1  and line.find("ReadPosRankSum=") !=-1:
                            QDlist1+=[float(line[line.find("QD=")+3:line.find("ReadPosRankSum=")-1])]
                        elif line.startswith("#")==False and line.find("QD=")!=-1  and line.find("ReadPosRankSum=") ==-1:
    ...
    d2={"Gene":Genelist2, "POS":POSlist2, "Ref":Reflist2, "Alt":Altlist2, "AAref":AAlist21, "AAalt":AAlist22, "Depth":depthlist2up, "AF": AFlist1, "Mutation": mutationlist1, "QD": QDlist1}
    df2=pd.DataFrame(data=d2)
    #df.to_csv('snpreport4.csv', index=False,) 
    df2.to_csv('snpreport5.csv', index=False,) 
